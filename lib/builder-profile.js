...
export function buildInterviewRecap(builder) {
  ...
  const bullets = [
    ...
    builder.projects.length
      ? `${builder.projects.length} ${builder.projects.length === 1 ? "build" : "builds"} already captured in the studio.`
      : "",
  ].filter(Boolean);
...
export function buildProjectInterviewQuestion(builder, activeProject = null) {
  ...
  if (builder.projects[0]?.title) {
    return `Let's start with ${builder.projects[0].title}, unless there's a different build you want me to begin with. What does it do, and why does it matter?`;
  }
