...
export function syncBuilderFromProfile(builder) {
  ...
  const hasSignal =
    countAnsweredProfileFields(normalizedBuilder.profile) > 0 ||
    normalizedBuilder.toolStack.regular.length > 0 ||
    normalizedBuilder.toolStack.familiar.length > 0 ||
    normalizedBuilder.github.profileUrl ||
    normalizedBuilder.github.repoUrls.length;
