...
export function createPostgresStore() {
  return {
    ...
    async publishBuilder(userId) {
      ...
      const publishedData = { ...current.draft };
      delete publishedData.chatHistory;

      await query(
        "UPDATE builder_profiles SET published_json = $1, published_at = $2, updated_at = NOW() WHERE user_id = $3",
        [JSON.stringify(publishedData), publishedAt, userId],
      );
    ...
    async insertAnalyticsEvent({ id, slug, eventType, visitorId, metadata }) {
      await ensureTables();
      await query(
        "INSERT INTO analytics_events (id, slug, event_type, visitor_id, metadata) VALUES ($1, $2, $3, $4, $5)",
        [id, slug, eventType, visitorId, metadata],
      );
    },

    async getAnalyticsEvents(slug, days = 30) {
      await ensureTables();
      const result = await query(
        `SELECT DATE(created_at) AS date, event_type, COUNT(*)::int AS count
         FROM analytics_events
         WHERE slug = $1 AND created_at >= NOW() - MAKE_INTERVAL(days => $2)
    ...
    async getAnalyticsVisitorCount(slug, days = 30) {
      await ensureTables();
      const result = await query(
