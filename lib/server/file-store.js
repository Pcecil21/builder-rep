...
function normalizeBuilderRecord(record) {
  ...
  const draft = normalizeBuilder(record.draft);
  const published = record.published ? normalizeBuilder(record.published, draft) : null;

  return {
    ...record,
    slug: record.slug || draft.slug,
    draft: { ...draft, slug: record.slug || draft.slug },
    published,
  };
}
...
export function createFileStore() {
  return {
    ...
    async getBuilderRecordByUserId(userId) {
      const state = await readState();
      return normalizeBuilderRecord(state.builders.find((builder) => builder.userId === userId) ?? null);
    },
    ...
    async resetBuilderForUser({ userId, email }) {
      ...
      const resetDraft = {
        ...
        id: record.draft?.id ?? `builder-${userId}`,
        slug: record.slug,
      };

      record.draft = resetDraft;
    ...
    async publishBuilder(userId) {
      ...
      const published = structuredClone(record.draft);
    ...
    async getPublishedBuilderBySlug(slug) {
      ...
      return normalizeBuilderRecord(record)?.published ?? null;
    },
