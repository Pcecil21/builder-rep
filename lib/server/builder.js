...
function normalizeProject(project, index) {
  ...
  return {
    ...
    buildType: normalizeBuildType(raw.buildType),
    ...
    visuals: Array.isArray(raw.visuals)
      ? raw.visuals.map((visual, visualIndex) => ({
          title: asString(visual?.title, `Visual ${visualIndex + 1}`),
          description: asString(visual?.description, ""),
        }))
      : [],
  };
}
