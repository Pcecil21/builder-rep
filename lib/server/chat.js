...
function fallbackStudio({ builder, userText, stage, history }) {
  if (stage === "profile-refine") {
    return {
      reply: "Captured. That gives Chuckie better language to work with.",
      builderPatch: {
        shortBio: userText,
        longerIntro: builder.longerIntro,
        featuredIntroLine: builder.featuredIntroLine,
  ...
  if (stage === "build-refine") {
    return {
      reply: "Captured. That gives the build a clearer case-file read.",
      projectPatch: {
        shortDescription: userText.slice(0, 160),
        longDescription: userText,
        whatItIs: userText,
        whyItMatters: userText,
        whatItDemonstrates: userText,
        whyBuiltThisWay: userText,
        tags: themeCandidatesFromText(userText).map((item) => item.toLowerCase()),
      },
    };
  }
...
function buildStudioInstructions(stage, builder, questionId = "") {
  return [
    ...
    stage === "discovery"
      ...
      : stage === "profile-refine"
        ? "You are refining the builder profile from a specific answer. Rewrite the relevant profile fields crisply."
      : stage === "build-refine"
        ? "You are refining a single build from a specific answer. Rewrite only the relevant build fields crisply."
    ...
    questionId ? `The targeted question id is ${questionId}.` : "",
  ].join("\n");
}
...
async function handleStudio(payload) {
  const { builder, history, userText, stage, currentProject, questionId } = payload;
    ...
    properties: {
      ...
      builderPatch: {
        ...
        properties: {
          ...
          longerIntro: { type: "string" },
          featuredIntroLine: { type: "string" },
          themes: {
            type: "array",
            items: { type: "string" },
            maxItems: 6,
        ...
        required: ["shortBio", "longerIntro", "featuredIntroLine", "themes"],
      },
    },
    required: ["reply", "builderPatch"],
  };
  ...
  const buildRefineSchema = {
    ...
    required: ["reply", "projectPatch"],
  };

  try {
    const response = await callOpenAI({
      instructions: buildStudioInstructions(stage, builder, questionId),
      schemaName:
        stage === "discovery"
