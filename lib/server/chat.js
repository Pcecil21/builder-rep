...
function fallbackStudio({ builder, userText, stage, history, focusField }) {
  ...
  if (stage === "profile-refine") {
    return {
      reply: "That helps. I have a sharper read on how to talk about you now.",
  ...
  if (stage === "build-refine") {
    return {
      reply: "That helps. I have a clearer read on this build now.",
      knowledgeNote: userText,
    };
  }
...
function buildStudioInstructions(stage, builder, focusField) {
  return [
    ...
    stage === "onboarding-interview"
      ? "You are in the onboarding interview. Reflect back what you learned, summarize the user's answer into the requested profile field, and ask the next best question."
      : stage === "discovery"
      ? "You are in the discovery interview. Infer framing, themes, and prompt starts."
      : stage === "profile-refine"
        ? "The builder chose My Background. Return a concise knowledgeNote that should be appended to the background What Chuckie Knows field. reply should briefly say what Chuckie now understands."
      : stage === "build-refine"
        ? "The builder chose My Agents/Projects. Return a concise knowledgeNote that should be appended to the selected build's What Chuckie Knows field. reply should briefly say what Chuckie now understands."
    ...
    stage === "onboarding-interview" && focusField
      ? `The current interview focus is ${focusField}. Use that field for the summary.`
      : "",
  ].join("\n");
}
...
async function handleStudio(payload) {
  ...
  try {
    const response = await callOpenAI({
      instructions: buildStudioInstructions(stage, builder, focusField),
      schemaName:
        stage === "onboarding-interview"
      ...
      input: JSON.stringify({
        ...
        suggestedPrompt:
          stage === "onboarding-interview"
            ? buildInterviewQuestion(focusField ?? getNextInterviewField(builder), builder)
            : "",
        activePrompts: buildKnowledgeGapPrompts(builder).map((prompt) => prompt.prompt),
      }),
    });

    return response ?? fallbackStudio({ builder, userText, stage, history, focusField });
  } catch {
    return fallbackStudio({ builder, userText, stage, history, focusField });
  }
}
