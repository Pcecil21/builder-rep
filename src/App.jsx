...
import BuilderSetup from "./components/BuilderSetup";
import {
  BrowseProjectRow,
  LinkOutObject,
  ProjectDetailObject,
  ShowcaseObject,
  ThinkingObject,
} from "./components/RichObjects";
import { demoBuilder } from "./data/demoBuilder";
import { requestViewerTurn } from "./lib/api";
import {
    ...
    <header className="topbar">
      <div className="brand">
        ...
        <div>
          ...
          <span>NLW's builder representative</span>
        </div>
      </div>
      <div className="topbar-actions">
        <button type="button" className="ghost-button" onClick={() => onToggleMode("builder")}>
          Builder Studio
        </button>
        <button type="button" className="ghost-button" onClick={() => onToggleMode("browse")}>
...
export default function App() {
  const [builder, setBuilder] = useState(() => demoBuilder);
  const [mode, setMode] = useState("chat");
  const [browseProjectId, setBrowseProjectId] = useState(null);
  const [setupProjectId, setSetupProjectId] = useState(() => demoBuilder.projects[0]?.id ?? null);
  ...
  const handleSelectProject = async (projectId) => {
    ...
    await appendMessages([
      ...
      {
        role: "assistant",
        text: "That's the full picture. Ask me anything about the architecture or decisions behind it.",
      },
    ]);
  };
  ...
  const createProject = (projectDraft) => {
    ...
    const project = {
      ...
      visuals: projectDraft.visuals ?? [{ title: "Project visual", description: "Visual reference" }],
      artifacts: projectDraft.artifacts ?? [],
    };

    setBuilder((current) => ({ ...current, projects: [...current.projects, project] }));
    setSetupProjectId(id);
    return id;
  };
      ...
      <main className="main-area">
        {mode === "chat" ? (
          ...
          <div className="builder-frame">
            <BuilderSetup
              builder={builder}
              selectedProjectId={setupProjectId}
              onSelectProject={setSetupProjectId}
        ...
        <div className="composer-shell">
          ...
          <div className="composer-row">
            <input
              ...
              placeholder="Ask Chuckie anything..."
            />
            <button type="button" className="send-button" onClick={() => sendUserMessage(input)} disabled={!input.trim() || typing}>
              ↑
            </button>
          </div>
        </div>
